...
class HQQSVDLinear(torch.nn.Module):
    def __init__(
        self, W_q, svd_up, svd_down, scale, zero, bias, nbits
    ):
        super().__init__()
        self.in_features = svd_down.shape[1]
        self.out_features = svd_up.shape[0]
        self.svd_rank = svd_down.shape[0]
        self.group_size = self.out_features // scale.shape[1]
        self.n_groups = scale.shape[1]
        self.q_shape = torch.Size((self.in_features, self.n_groups, self.group_size))
        self.o_shape = torch.Size((self.out_features, self.in_features))
        self.W_q = torch.nn.Parameter(W_q, False)
        ...
        self.zero = torch.nn.Parameter(zero, False)
    ...
    def from_linear(
        ...
        return cls(
            W_q, svd_up, svd_down, scale, zero, linear.bias, nbits
        )
